...
export default async function EventDetailPage({ params }: PageProps) {
  ...
  const related = events
    ...
    .sort((a, b) => {
      if (a.category === event.category && b.category !== event.category)
        return -1;
      if (a.category !== event.category && b.category === event.category)
        return 1;
      return 0;
                ...
                <Card className="rounded-sm bg-ink text-cream shadow-sharp">
                  <CardContent className="space-y-4 pt-2">
                    ...
                    <p className="text-sm text-cream/80">
                      Contact us to reserve your spot for this event. All
                      programs are provided at no cost to veterans.
                    </p>
                ...
                <Card className="rounded-sm ring-1 ring-border shadow-sharp">
                  ...
                  <CardContent className="space-y-4 text-sm">
                    <div>
                      ...
                      <Badge
                        className={`mt-1 ${categoryColors[event.category]}`}
                      >
                        {categoryLabels[event.category]}
                      </Badge>
                    </div>
