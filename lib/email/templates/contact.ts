...
export async function sendContactNotification(
	...
	const body = `
		...
		<div style="background: #f7fafc; border-left: 4px solid #c0392b; padding: 12px 16px; margin: 16px 0;">
			<p style="margin: 0; font-weight: bold; font-size: 16px;">${escapeHtml(params.subject)}</p>
			<p style="margin: 2px 0; color: #4a5568;"><strong>From:</strong> ${escapeHtml(params.senderName)} (${escapeHtml(params.senderEmail)})</p>
			${phoneLine}
		</div>
